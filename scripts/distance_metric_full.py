...
def main(system, cutoff, sample, overwrite=True):
    ...
    for i, f in enumerate(files):
        ...
        normalize = tmp.mean(1) / tmp.mean(1).sum()
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", dest="system", type=str, help="4beads or phospate file")

    parser.add_argument("-c", dest="cutoff", type=int, help="config file")

    parser.add_argument("-z", dest="sample", type=int, help="sample size")

    parser.add_argument(
        "-ow", dest="overwrite", type=str2bool, default=False, help="overwrite"
    )
